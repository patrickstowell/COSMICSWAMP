...
class plugin(base_plugin):
    ...
    def build_widget(self, store):
        ...
        layout = html.Div([
            html.Div([
                html.Div(id="orion-type-set-dummy"),
                html.Div(id="orion-type-set-label", children="Entity Select :  ", className="input-widget-label"),
                dcc.Input(id="orion-type-set-input", debounce=True, value=store["type"], className="input-widget"),
    ...
    def register_widget(self, app):
        ...
        @app.callback( 
                    ...
                    Input("orion-type-set-input","value")
                )
        def set_entity(path):
